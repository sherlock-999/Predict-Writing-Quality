"""
Inference: Predict Essay Scores — Kaggle Submission (v5 features)
==================================================================
Self-contained script: all feature engineering logic is inlined.
No local module imports required.

Feature set: v5_features (206 features = v4 152 + 54 new)
  ...
  - Full up_event counts (16 events — was 3 in v4)
  - n_unique_activity, n_unique_up_event (new in FEATURE_COLS)
  - input_word_length_median (new)
  - action_time_min, up_time_{mean,std,median,q1,q3} (new)
  - cursor_position_min, word_count_{min,mean,max,sum} (new)
  - Per-word normalisations for all new counts

The TF-IDF SVD pipeline must be fitted on the training corpus and saved
as tfidf/tfidf_svd.pkl within MODELS_DIR.  It is loaded from TFIDF_PATH
at inference time so train and test use the same transformer.

Paths (Kaggle notebook environment):
    DATA_DIR    = /kaggle/input/competitions/linking-writing-processes-to-writing-quality
    MODELS_DIR  = /kaggle/input/datasets/sherlocked999/writing-quality-lightgbm-v5
...
# =============================================================================
# FEATURE COLUMNS  (v5 — 206 features)
# =============================================================================
...
FEATURE_COLS = [
    ...
    # ── 9. UP EVENT COUNTS  (v5 new — all 16 reference events) ───────────────
    'up_event_q_cnt',
    'up_event_Space_cnt',
    ...
    # ── 13. TF-IDF SVD (char bigrams on reconstructed text) ──────────────────
...
def _count_features(essay: pd.DataFrame) -> dict:
    ...
    # Up event counts (v5: all 16 reference events)
    for value, key in [
        ('q',            'up_event_q_cnt'),
    ...
    feats['n_unique_up_event']    = int(ue.nunique())    # v5 new in FEATURE_COLS
...
def _timing_features(essay: pd.DataFrame) -> dict:
    ...
    feats['word_count_max']    = float(wc.max())     # v5 new
...
print("Computing per-essay features (v5)...")
