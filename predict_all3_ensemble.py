"""
Ensemble Inference: LightGBM + XGBoost + CatBoost → submission.csv  (Kaggle Submission)
=========================================================================================
Self-contained script: all feature engineering logic is inlined.
No local module imports required.

Loads 50 LightGBM + 50 XGBoost + 50 CatBoost fold models, averages predictions
within each family, then blends the three using inverse-OOF-RMSE weighting.
...
Feature set: v6_features (226 features = v5 206 + 20 new)
  v5 base (206): extended activity/text_change/down_event/up_event counts,
    ...
    per-word normalizations for all new counts
  v6 new (20): Event TF-IDF SVD — word 1–5-gram on down_event sequence

Two shared TF-IDF SVD pipelines loaded at inference time:
...
Paths (Kaggle notebook environment):
    ...
    LGBM_MODELS_DIR     = /kaggle/input/datasets/sherlocked999/v6-features-lgbm/v6_features_lgbm
    XGB_MODELS_DIR      = /kaggle/input/datasets/sherlocked999/xgb-model/xgb_model
...
OOF_RMSE_LGBM     = 0.6024
OOF_RMSE_XGB      = 0.6054
OOF_RMSE_CATBOOST = 0.5984
...
# =============================================================================
# SECTION 1 – LOAD SHARED TF-IDF PIPELINES
# =============================================================================
...
# XGBoost
xgb_paths = sorted(glob.glob(f'{XGB_MODELS_DIR}/xgb_s*_fold*.json'))
if not xgb_paths:
    raise FileNotFoundError(
        f"No XGBoost models found in {XGB_MODELS_DIR}/\n"
        "Upload xgb_s42_fold1.json … xgb_s4_fold10.json to the dataset."
    )
xgb_models = []
for p in xgb_paths:
    m = xgb.Booster()
    m.load_model(p)
    xgb_models.append(m)
print(f"Loaded {len(xgb_models)} XGBoost models from {XGB_MODELS_DIR}/")
...
w_total    = w_lgbm + w_xgb + w_catboost

preds = (w_lgbm * lgbm_preds + w_xgb * xgb_preds + w_catboost * cb_preds) / w_total
preds = np.clip(preds, 0.5, 6.0)

print(f"\nBlend weights — "
      f"LightGBM: {w_lgbm/w_total:.3f} | "
      f"XGBoost: {w_xgb/w_total:.3f} | "
      f"CatBoost: {w_catboost/w_total:.3f}")
