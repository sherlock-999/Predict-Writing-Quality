"""
Inference: Predict Essay Scores — Kaggle Submission (XGBoost, v6 features)
===========================================================================
...
Paths (Kaggle notebook environment):
    ...
    MODELS_DIR       = /kaggle/input/datasets/sherlocked999/writing-quality-xgboost-v6
...
def compute_features(logs: pd.DataFrame, tfidf_pipeline, event_tfidf_pipeline) -> pd.DataFrame:
...
# =============================================================================
model_paths = sorted(glob.glob(f'{MODELS_DIR}/xgb_s*_fold*.json'))
if not model_paths:
    raise FileNotFoundError(
        f"No fold models found in {MODELS_DIR}/\n"
        "Upload xgb_s*_fold*.json models to the dataset."
    )
...
for p in model_paths:
    ...
    m.load_model(p)
    models.append(m)
print(f"Loaded {len(models)} XGBoost models from {MODELS_DIR}/")
...
X_test  = xgb.DMatrix(test_df[FEATURE_COLS].values, feature_names=FEATURE_COLS)
print(f"  Feature matrix: {test_df.shape[0]} essays × {len(FEATURE_COLS)} features")
