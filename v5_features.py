"""
v5 Features: Linking Writing Process to Writing Quality
========================================================
Base: v4_features (152 features)

New features added in v5 — completes the reference feature set:

  Extended activity counts:
    activity_Replace_cnt, activity_Paste_cnt
    replace_per_word, paste_per_word

  Extended text_change counts:
    text_change_doublequote_cnt, text_change_semicolon_cnt,
    text_change_equals_cnt, text_change_slash_cnt,
    text_change_backslash_cnt, text_change_colon_cnt
    + per_word versions of each

  Extended down_event counts:
    down_event_ArrowDown_cnt, down_event_ArrowUp_cnt,
    down_event_singlequote_cnt, down_event_Delete_cnt,
    down_event_Unidentified_cnt
    + per_word versions of each

  Full up_event counts (was 3 in v4, now all 17 reference events):
    up_event_{q, Space, Backspace, Shift, ArrowRight, Leftclick,
              ArrowLeft, period, comma, ArrowDown, ArrowUp, Enter,
              CapsLock, singlequote, Delete, Unidentified}_cnt

  n_unique:
    n_unique_activity  (new)
    n_unique_up_event  (was computed in v4 but missing from FEATURE_COLS)

  Extended input word stats:
    input_word_length_median  (new)

  Extended numerical aggregations:
    action_time_min           (new)
    up_time_{mean,std,median,q1,q3}  (new — v4 only had min/max)
    cursor_position_min       (new)
    word_count_{mean,min,max,sum}    (new — v4 only had std/median/q1/q3)

Total: 152 (v4) + 54 new = 206 features

Two-pass design (same as v4):
...
def _count_features(essay: pd.DataFrame) -> dict:
    ...
    # ── Activity counts (v4: Input, Remove/Cut, Nonproduction)
    #                   (v5 adds: Replace, Paste)
    for value, key in [
        ('Input',         'activity_Input_cnt'),
        ('Remove/Cut',    'activity_RemoveCut_cnt'),
        ('Nonproduction', 'activity_Nonproduction_cnt'),
        ('Replace',       'activity_Replace_cnt'),      # v5 new
        ('Paste',         'activity_Paste_cnt'),         # v5 new
    ]:
        feats[key] = int((act == value).sum())

    # ── Text change counts (v4: q, space, period, comma, newline, quote, dash, question)
    #                       (v5 adds: doublequote, semicolon, equals, slash, backslash, colon)
    for value, key in [
        ('q',   'text_change_q_cnt'),
        ...
        ('"',   'text_change_doublequote_cnt'),  # v5 new
        (';',   'text_change_semicolon_cnt'),    # v5 new
        ('=',   'text_change_equals_cnt'),       # v5 new
        ('/',   'text_change_slash_cnt'),         # v5 new
        ('\\',  'text_change_backslash_cnt'),    # v5 new
        (':',   'text_change_colon_cnt'),         # v5 new
    ]:
        feats[key] = int((tc == value).sum())

    # ── Down event counts (v4: 11 events)
    #                     (v5 adds: ArrowDown, ArrowUp, singlequote, Delete, Unidentified)
    for value, key in [
        ('q',            'down_event_q_cnt'),
        ...
        ('ArrowDown',    'down_event_ArrowDown_cnt'),    # v5 new
        ('ArrowUp',      'down_event_ArrowUp_cnt'),      # v5 new
        ("'",            'down_event_singlequote_cnt'),  # v5 new
        ('Delete',       'down_event_Delete_cnt'),       # v5 new
        ('Unidentified', 'down_event_Unidentified_cnt'), # v5 new
    ]:
        feats[key] = int((de == value).sum())

    # ── Up event counts (v4: only q, comma, Shift)
    #                   (v5: all 17 reference events)
    for value, key in [
        ('q',            'up_event_q_cnt'),
        ('Space',        'up_event_Space_cnt'),          # v5 new
        ('Backspace',    'up_event_Backspace_cnt'),      # v5 new
        ('Shift',        'up_event_Shift_cnt'),
        ('ArrowRight',   'up_event_ArrowRight_cnt'),     # v5 new
        ('Leftclick',    'up_event_Leftclick_cnt'),      # v5 new
        ('ArrowLeft',    'up_event_ArrowLeft_cnt'),      # v5 new
        ('.',            'up_event_period_cnt'),          # v5 new
        (',',            'up_event_comma_cnt'),
        ('ArrowDown',    'up_event_ArrowDown_cnt'),      # v5 new
        ('ArrowUp',      'up_event_ArrowUp_cnt'),        # v5 new
        ('Enter',        'up_event_Enter_cnt'),           # v5 new
        ('CapsLock',     'up_event_CapsLock_cnt'),       # v5 new
        ("'",            'up_event_singlequote_cnt'),    # v5 new
        ('Delete',       'up_event_Delete_cnt'),         # v5 new
        ('Unidentified', 'up_event_Unidentified_cnt'),   # v5 new
    ]:
        feats[key] = int((ue == value).sum())

    # ── Diversity counts
    feats['n_unique_activity']    = int(act.nunique())   # v5 new
    feats['n_unique_text_change'] = int(tc.nunique())
    feats['n_unique_down_event']  = int(de.nunique())
    feats['n_unique_up_event']    = int(ue.nunique())    # computed in v4, now in FEATURE_COLS

    return feats


# =============================================================================
# STEP 2 — INPUT WORD FEATURES  (extended from v4: adds median)
# =============================================================================
...
def _input_word_features(essay: pd.DataFrame) -> dict:
    ...
    return {
        ...
        'input_word_length_median':  np.median(lengths)    if lengths else 0.0,  # v5 new
        'input_word_length_skew':    float(_skew(lengths)) if lengths else 0.0,
    }


# =============================================================================
# STEP 3 — TIMING FEATURES  (extended from v4)
# =============================================================================
...
def _timing_features(essay: pd.DataFrame) -> dict:
    ...
    feats['action_time_min']    = float(at.min())    # v5 new
    ...
    feats['up_time_mean']   = float(ut.mean())    # v5 new
    feats['up_time_std']    = float(ut.std())     # v5 new
    feats['up_time_median'] = float(ut.median())  # v5 new
    feats['up_time_q1']     = float(ut.quantile(0.25))  # v5 new
    feats['up_time_q3']     = float(ut.quantile(0.75))  # v5 new
    feats['up_time_max']    = float(ut.max())

    cp = essay['cursor_position']
    feats['cursor_position_min']    = float(cp.min())    # v5 new
    ...
    wc = essay['word_count']
    feats['word_count_min']    = float(wc.min())     # v5 new
    feats['word_count_mean']   = float(wc.mean())    # v5 new
    feats['word_count_max']    = float(wc.max())     # v5 new (computed in v4 but not in FEATURE_COLS)
    feats['word_count_sum']    = float(wc.sum())     # v5 new
...
# =============================================================================
# STEPS 14–15 — READABILITY + REVISION TIMING  (unchanged from v4)
# =============================================================================
...
# =============================================================================
# COMPUTE FEATURES  (same two-pass design as v4)
# =============================================================================

def compute_features(logs: pd.DataFrame, tfidf_pipeline=None):
    """
    Compute per-essay v5 features from raw keystroke logs.
    ...
    tfidf_pipeline : fitted sklearn Pipeline or None
        None → fit on corpus (training). Provided → transform only (inference).
...
# =============================================================================
# FEATURE COLUMNS  (v4 152 + 54 new = 206 features)
# =============================================================================
...
FEATURE_COLS = [
    ...
    'input_word_length_median',   # v5 new
    'input_word_length_skew',
    'word_count_min',             # v5 new
    'word_count_mean',            # v5 new
    'word_count_max',             # v5 new
    'word_count_sum',             # v5 new
    'word_count_std',
    'word_count_median',
    'word_count_q1',
    'word_count_q3',
    ...
    'question_per_word',
    'quote_per_word',
    'doublequote_text_per_word',  # v5 new
    'semicolon_text_per_word',    # v5 new
    'equals_text_per_word',       # v5 new
    'slash_text_per_word',        # v5 new
    'backslash_text_per_word',    # v5 new
    'colon_text_per_word',        # v5 new
    'singlequote_per_word',       # v5 new (down_event "'")

    # ── 5. TYPING SPEED & FLUENCY ─────────────────────────────────────────────
    'keys_per_second',
    'action_time_min',            # v5 new
    'action_time_sum',
    'action_time_mean',
    'action_time_std',
    'action_time_median',
    ...
    'space_per_word',
    'space_text_per_word',
    'n_unique_activity',          # v5 new
    'n_unique_text_change',
    'n_unique_down_event',
    'n_unique_up_event',          # v5 new (was computed in v4, now in FEATURE_COLS)
    ...
    'input_per_word',
    'replace_per_word',           # v5 new
    'paste_per_word',             # v5 new
    ...
    'arrowleft_per_word',
    'arrowright_per_word',
    'arrowdown_per_word',         # v5 new
    'arrowup_per_word',           # v5 new
    'unidentified_per_word',      # v5 new
    'down_time_min',
    'down_time_mean',
    'down_time_std',
    'down_time_median',
    ...
    'up_time_mean',               # v5 new
    'up_time_std',                # v5 new
    'up_time_median',             # v5 new
    'up_time_q1',                 # v5 new
    'up_time_q3',                 # v5 new
    'up_time_max',

    # ── 9. UP EVENT COUNTS  (v5 new — all 17 reference events) ───────────────
    'up_event_q_cnt',
    'up_event_Space_cnt',
    ...
    # ── 10. RAW ACTIVITY / EVENT COUNTS  (v5 new raw counts) ─────────────────
    ...
    # ── 11. READABILITY & STRUCTURAL CONSISTENCY  (from v4) ───────────────────
    'ari_score',
    'coleman_liau_score',
    'sent_len_std',
    'sent_len_cv',
