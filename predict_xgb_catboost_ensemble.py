"""
Ensemble Inference: XGBoost + CatBoost → submission.csv  (Kaggle Submission)
==============================================================================
Self-contained script: all feature engineering logic is inlined.
No local module imports required.

Loads 50 XGBoost + 50 CatBoost fold models, averages predictions within each
...
print(f"\nXGBoost  — mean: {xgb_preds.mean():.3f} | std: {xgb_preds.std():.3f}")
...
w_total    = w_xgb + w_catboost

preds = (w_xgb * xgb_preds + w_catboost * cb_preds) / w_total
